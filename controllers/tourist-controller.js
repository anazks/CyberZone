...
const profileUpdate = async (req,res)=>{
    try {
        let compnay =  req.session.tourist;
        let data = req.body;
        console.log(company,"comany")
        data.company_id = compnay._id;
         let company = await companyModel.create(data)
...
const doLogin = async (req, res) => {
    try {
        ...
        const tourist = await TouristModel.findOne({ email: req.body.email,status :true });
        if (tourist) {
...
module.exports = { profileUpdate,forGotPage,newPassword,searchByCategory,getplace, getSignupPage, doSignup, getLoginPage, doLogin, getHomePage, logout, searchHotel, searchHospital, searchTheater, searchLibrary }