...
const RentModel = require('../models/Rent_new')
const Rent_new = require("../models/Rent_new")
//login --------------------------------------------------
...
const adminDashboard = async (req, res) => {
    ...
    try {
        let count = {};
        count.hotel = await complaintModel.count();
        count.hospital = await RentModel.count();
        count.college = await paymentModel.count();
...
const viewAllStudents = async (req, res) => {
    try {
        const students = await paymentModel.find({})
...
const viewAllJobSeekers = async (req, res) => {
    try {
        const jobSeekers = await complaintModel.find({})
...
const approveCompany = async (req, res) => {
    try {
        ...
        // Use the id directly in the findByIdAndUpdate method
        let updated = await TouristModel.findByIdAndUpdate(id, { status: true }, { new: true });
        
        // Log the updated document to see if it contains the changes
...
const viewAllHospital = async (req, res) => {
    try {
        const Rent = await Rent_new.find({})
        console.log(Rent)
        res.render("admin/view-all-hospitals", { Rent })
    } catch (error) {
        console.log(error)
    }
}
...
const viewAllIndustries = async (req, res) => {
    try {
        const industries = await touristModel.find({})
